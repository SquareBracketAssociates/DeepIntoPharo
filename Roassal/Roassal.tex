...
A layout is applied to  a collection of elements to arrange them. Layouts inherits from the \textbf{ROLayout} class. To apply  layout use the \#on: selector with a collection of ROElement's, as shown in \figref{ROGrid}.
...
There are several event classes to deal with mouse events: \textbf{ROMouseClick}, \textbf{ROMouseMove}, \textbf{ROMouseEnter} and \textbf{ROMouseLeave}, among others. To deal with key pressing, \textbf{ROKeyDown} is the event class to be used.

\vp{explain the following code}
...
As previously presented, a ROElement can respond to events by adding it interactions. The root class of all the interactions is \textbf{ROInteraction}.

\vp{Say a bit more about interactions a clasification? 1) apply directly to an object with defined callbacls (RODraggable, ROPopup) and 2) Animations}
From the interactions availables in Roassal, in this chapter we will present 3
...
view open.
  \end{code}
   ...
   \begin{minipage}[t]{1\textwidth}
	 \vspace{0pt} \raggedright
       \centering
		\includegraphics[width=0.7\textwidth]{popupView}
   \end{minipage}
\label{fig:popupView}
\caption{ROPopupView}
\end{figure} 
