...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    \#tab asParser            & Parses a tabulator character.                       \\
...
\item[Tally] shows how many times was particular parser called during
  ...
  performance of your parser (such as  \figref{pp/tally}).
...
\item[Profile] shows how much time was spent in particular parser
  ...
  your parser  (such as  \figref{pp/profile}).
...
%  \jk{I added example further}
  ...
  or use memoized parser.
  We will hava a detailed look on a backtracking issue in the 
  following section.
...
\subsection{Debugging example}
  ...
  The \clsind{BacktrackingParser} was designed to accept input corresponding to the
  regular expressions '\ct{a*b}' and '\ct{a*c}'.
  The parser gives us correct results, but there is a problem with a performance.
  The \clsind{BacktrackingParser} performs a lot of backtracking.
...
\begin{script}{Parser that accepts regular expressions '\ct{a*b}' or '\ct{a*c}'. Unfortunately, it performs a lot of backtracking as well. \label{scr:pp/backtrackingParser}}
...
But the progress depicted in \figref{pp/example-progressAaac} looks worse.
The $input_c$  is parsed with a lot of backtracking and in much longer time. 
...
It is much less than 110 invocations for the parser \ct{b} and 55 invocation for the parser \ct{a} in case of $input_c$.

\begin{figure}
  \begin{center}
    \includegraphics[width=\linewidth]{example-progressAaab}
    \caption{Progress of the \clsind{BacktrackingParser} for $input_b$. \label{fig:pp/example-progressAaab}}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Progress of the \clsind{BacktrackingParser} for $input_c$. \label{fig:pp/example-progressAaac}}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Tally output of the \clsind{BacktrackingParser} for $input_b$. \label{fig:pp/example-tallyAaab}}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Tally output of the \clsind{BacktrackingParser} for $input_c$. \label{fig:pp/example-tallyAaac}}
  \end{center}
\end{figure}

What is happening? 
We can see there is some problem with $input_c$, but what is the problem?
...
We can see that in each step, one '\ct{a}' is consumed and the parser \ct{ab} is invoked again as long as it reaches the '\ct{b}'.
On the other hand, the debugger window for $input_c$ as depicted in \figref{pp/example-debugAaac} looks worse.
There is a progress within the \ct{p} -> \ct{ab} -> \ct{ap} -> \ct{p} loop, but the parser \ct{ab} fails in the each repetition of the loop. 
And since the parser \ct{ab} fails after it reads all the string to the end and sees '\ct{c}' instead of '\ct{b}', we have localized the cause of the backtracking.
We know the problem now, so what can we do?
Maybe we should update \clsind{BacktrackingParser} parser so that the '\ct{a*c}' strings are parsed in a similar way as the '\ct{a*b}' strings.
...
\begin{figure}
  \begin{center}
    ...
    \caption{Debugging output of the \clsind{BacktrackingParser} for $input_b$. \label{fig:pp/example-debugAaab}}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Debugging output of the \clsind{BacktrackingParser} for $input_c$. \label{fig:pp/example-debugAaac}}
  \end{center}
\end{figure}


\begin{script}{Parser that accepts regular expressions '\ct{a*b}' or '\ct{a*c}'. The first improved version. \label{scr:pp/backtrackingParser-update-1}}
PPCompositeParser subclass: #BacktrackingParser
...
As we check the progress for $input_c$ as depicted in \figref{pp/example-progressAaac-update-1}, we can see significant improvement.
Improvement is confirmed by the tally window as well.
For $input_c$, the tally shows only 20 invocations of the parser \ct{b} and 9 invocations of the parser \ct{a} (see \figref{pp/example-tallyAaac-update-1}).
This is very good improvement compared to the 110 invocations of the parser \ct{b} and 55 invocations of the parser \ct{a} in an original version of \clsind{BacktrackingParser}.

\begin{figure}
  \begin{center}
    \includegraphics[width=\linewidth]{example-progressAaac-update-1}
    \caption{Progress of the \clsind{BacktrackingParser} after the first update for $input_c$. \label{fig:pp/example-progressAaac-update-1}}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Tally of the \clsind{BacktrackingParser} after the first update for $input_c$. \label{fig:pp/example-tallyAaac-update-1}}
  \end{center}
\end{figure}

Yet, we might try to do even better.
There is still one backtracking happening for $input_c$. 
It happens when the parser \ct{ab} tries to recognize the '\ct{a*b}' input, then it fails (and backtracks) so that the parser \ct{ac} can recognize the '\ct{a*c}' input.
What if we try to consume all the '\ct{a}'s and then we choose between '\ct{b}' and '\ct{c}' in the very end?
You can see such a modification of the \clsind{BacktrackingParser} in \scrref{pp/backtrackingParser-update-2}.
...
If we compare the tally windows from of the first and second versions (see \figref{pp/example-tallyAaac-update-2} and \figref{pp/example-tallyAaac-update-1}),we can see that second version is by 19 invocations (10 for a sequence parser and 9 for the second parser \ct{a}) for $input_c$. 

On the other hand, the number of parser invocations for $input_b$ increased by 18 (the \tabref{example-tally} summarizes the total number of invocations for particular versions of the \clsind{BacktrackingParser}).
Now, it is up to the developer to decide which grammar more suitable for his needs. 
It is better to use the second update in case the '\ct{a*b}' and '\ct{a*c}' strings occurs with the same probability as an input.
If we expect more '\ct{a*b}' strings as an input, the first version is better.




\begin{script}{Parser that accepts regular expressions '\ct{a*b}' or '\ct{a*c}'. Second improved version. \label{scr:pp/backtrackingParser-update-2}}
PPCompositeParser subclass: #BacktrackingParser
...
\begin{figure}
  \begin{center}
    ...
    \caption{Progress of the \clsind{BacktrackingParser} after the second update for $input_c$. \label{fig:pp/example-progressAaac-update-2}}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Tally of the \clsind{BacktrackingParser} after the second update for $input_c$. \label{fig:pp/example-tallyAaac-update-2}}
  \end{center}
\end{figure}
...
\begin{table}
  ...
  \end{tabular}
  \caption{Number of parser invocations for $input_b$ and $input_c$ depending on a version of the \clsind{BacktrackingParser}.}
...
But in the debugging example we saw that original version of the \clsind{BacktrackingParser} parsed $input_c$ of length 10 in 233 steps. 
And if you try to parser $longinput_c$ = '\ct{aaaaaaaaaaaaaaaaaaaac}' (length 20), you will see it takes 969 steps.
...
You need to use \ct{memoized} message to enable packrat parsing.
...
To give you an example with a packrat parser, let us return back to the \clsind{BacktrackingParser} once again (see \scrref{pp/backtrackingParser}).
As we have analyzed before, the problem was in the parser \ct{ab} that constantly fails in the \ct{p} -> \ct{ab} -> \ct{ap} -> \ct{p} loop.
...
With the minor modification of \clsind{BacktrackingParser} we got a linear parsing time (related to the length of an input)  with a factor around 6.
...
\begin{figure}
  \begin{center}
    ...
    \caption{Progress of the memoized version of the \clsind{BacktrackingParser}. \label{fig:pp/example-progressAaac-memo}}
  \end{center}
\end{figure}
...
\item To know if a string matches with the grammar, use the method \ct{matches:}.
