...
\item[First] shows first set of specified rule\dc{first set of what?}.
  The first set is useful for optimizing your grammar\dc{keep that for
    the end of the bullet and explain how the reader can optimize with
    this information}. It basically shows a list of rules that can be
  activated directly after the rule started\dc{so why do I see '(' and
    not 'parens' in the screenshot?}. As you can see on
  \figref{pp/browserFirst} the first set of \ct{prim} is either digit
  or opening parenthesis '\ct{(}'. This means that once you start
  parsing \ct{prim} the input should start with either digit or
  '\ct{(}'.
...
\end{script}
  ...
  First set is computed from definition of \ct{prim} depicted in
  \scrref{pp/expressionGrammrPrimAndParens}. Digit comes from
  \ct{number} and \ct{(} comes from \ct{parens} rule. In general,
  computation of first set could be complex and therefore
  \ct{PPBrowser} computes this information for us.
\item[Follow] shows follow set of specified rule \dc{please improve
    this bullet the same way you improve the previous one}. The follow
  set (similarly to first set) is useful for optimizing your grammar.
  As you can see on \figref{pp/browserFollow} the follow set of
  \ct{prim} is closing bracket character \ct{)}, star character
  \ct{*}, plus character \ct{+} or \ct{epsilon} (which states for
  empty string). This means that after you parse the \ct{prim} rule,
  the input continues with closing bracket, plus and star or input
  ends (because of \ct{epsilon}).

  Computation of follow set is even more complex than computation of
  first set and therefore \ct{PPBrowser} computes the follow set for
  us.\dc{why is this information important?}
...
\item[Debugger] shows a tree view of the steps that were performed
  ...
  by a particular step. For example, you can inspect how does the
  \clsind{PPJsonGrammar} work, what rules are called and in which
  order as is depicted in \figref{pp/debugger}. The grey rules are
  rules that failed. This usually happens for ordered choice and you
  can see example for the \ct{value} rule. At first, parser tried to
  parse string and number at position 0, but it failed so that it
  tried the \ct{object} parser which succeeded. \dc{isn't it possible
    to the same for the expression grammar? I think it would be
    more coherent with the rest of the section. If not, just delete my
    comment.}

\begin{figure}
  \begin{center}
    \includegraphics[width=.8\linewidth]{PPBrowserDebugger}
...
\item[Progress] visually shows how a parser consumes input. The x-axis
  represents how many characters where read in the input sample,
  ...
  steps. When multiple lines are visible, it means that the parser had
  ...
  go back five times to correctly parse the whole input sample: all
