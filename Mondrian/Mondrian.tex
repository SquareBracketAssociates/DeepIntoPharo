...
There are essentially two ways to work with Mondrian. Either by using the easel which provides an interactive scripting engine or directly by interacting with a view renderer \ja{what are the easel and the view renderer ? what is the difference ?}. We will first use Mondrian in its easiest way, by using the easel. To open an easel, you can either use the World menu (it should contains the entry ``Mondrian Easel'') or execute the expression:

\begin{code}{}
MOEasel open.
\end{code}

In the easel you have just opened, you can see two panels: the one on top is the visualization panel, the second one is the script panel \ja{give a screenshot, maybe replacing the first image}. In the script panel, enter the following code and press the \emph{generate} button:
...
Each number is linked with its double. Not all the doubles are visualized \ja{visible?}. For example, the double of 20 is 400, which is not part of the visualization. In that case, no edge is drawn. 
...
We will now visualize Smalltalk classes. In the remaining of this section, we will intensively use Smalltalk reflection \ja{is it explained before in the book ? else we should explain it in few words.} to make compelling examples. Let's visualize the hierarchy of classes contained in the Collection framework:
...
\ja{I just see that the name edgesFrom: is not logical... it should be edgesTo:}
...
\caption{Abstract classes are in gray and classes with the word ``Abstract'' in their name are in blue. \ja{this class has the same label as fig 1.6}}
\label{fig:abstractClasses}
\end{figure}

Similarly than with \ct{height:} and \ct{width:}, messages to defines color either takes a symbol, a block or a constant value as argument. The argument is evaluated against the domain object represented by the graphical element (the message \ct{moValue:} with the domain object is sent to the argument \ja{what ?}). 
The use of \ct{ifTrue:ifFalse:} is a bit cumbersome \ja{a synonym should be better :)}. Utilities methods are provided for that purpose to easily pick a color from a particular condition. The definition of the shape can simply be:
...
\figref{systemComplexity} shows the resulting picture. The message \ct{linearFillColor:within:} takes as first argument a block function that return a numerical value. The second argument is a group of elements that is used to scale the intensity of each node. The block function is applied to each element of the group. The greatest value is set to black. The fading scales from 0 to this maximum \ja{why THIS maximum ? in real, black is 0 and white is 255. Maybe you should write: from white (0 line of codes) to black (maximum lines of code)}. The maximum intensity is given by the maximum \ct{#numberOfLinesOfCode} can take for all the subclasses of \ct{Collection}. 
...
\caption{Boxes are classes and links are inheritance relationships. The amount of abstract method is indicated by the size of the class. A red class defines abstract methods and a pink class inherits from an abstract class solely.\ja{this figure has the same label as 1.4 and 1.6}}
\label{fig:abstractClasses}
\end{figure}
...
In the example, we used \ct{sortedAs:} to order the nodes representing methods. This method is defined on Collection and belongs to Mondrian \ja{why it belongs to mondrian ?}. To see example usages of \ct{sortedAs:}, browse its corresponding unit test:
\begin{code}{}
...
Moving a method will move the class instead. \ja{add a transition} As most operating systems, Mondrian offers multiple selection using the Ctrl or Cmd key. This default behavior is available for every nodes.
