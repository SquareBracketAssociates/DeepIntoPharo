...
Modern applications are often distributed on multiple devices and collaborate through a network\del{ to achieve some task}.
...
We first show through some examples how to use sockets for building both clients and servers. \ins{The notion of client and server are inherent to sockets: a server waits for a request emitted by a client.}
...
Sockets are used to \chg{achieve}{establish} a bidirectional communication\chg{. T}{: t}hey allow both sending and receiving data. 
...
Data reach\ins{es} the destination in the same order of their sending.
...
The second reason why UDP is qualified as unreliable is that the reception order of multiple datagrams from a single sender to \chg{some particular}{the} receiver may arrive in an arbitrary order.
...
We describe their life-cycle and how to use them to implement a server that can handle \del{multiple} concurrent connections.
...
\item Connect the socket to \chg{some}{a} server.
...
Note that to use the \ct!NetNameResolver! you need to have your machine connected to a network with a DNS\footnote{\emph{Domain Name System}: basically a directory that maps device names to their IP address.}\ins{, which should probably be the case}.
...
Message \ct!connectTo:port:! attempts to connect the socket to the server \chg{which}{using the} address and port \del{are} provided as parameters.
\nouryComment{SocketAddress can also store the port of the server. 
But, NetNameResolver does NOT support building socket address WITH port.} \ins{A port is a numerical value that comes with with the address to designated a service. In our case, 80 is the port expected by the HTTP server to receive a connection.}\alex{Noury, maybe you have a better explanation of what is a port}
...
Once \ins{the} connection is established, the client can send/receive data to/from the server.
...
Typically, the client sends some request to the server and then expects \chg{receiving some}{for a} response.
...
Next, we send the \ct!receiveData! to our client socket to \chg{make it wait for data reception}{read the answer}. \ins{Note that reading the answer is \emph{blocking}, meaning \ct{receiveData} returns when an response has been read.} Then, the contents of variable \ct!data! is processed.
...
\del{Once the interaction is over, either the server or the client can decide to close the socket.}
\chg{This can be done by sending the \ct!close! message to the socket.}{A socket is closed by sending \ct!close! to it.}
\ugh{The image where this message is evaluated will then send a close request to the other side}\alex{maybe this sentence is not necessary}.
...
Since, we intend to display the result of our query on the \ct!Transcript!, we state\alex{where?} that our client accepts texts with html format.
...
One socket is used for handling client connections, while the second serves for exchanging data with a particular client. \ins{Without having two sockets, the server could not handle more than one request. Even if you design a server that is supposed to receive one request, using two sockets is considered a good practice and avoid one to mess up with socket timeout.}

\subsection{TCP Socket Server Life-cycle}
The life-cycle of a TCP server in \pharo \alex{only in Pharo? I think this is similar in other platform right?}  has 5 steps:
\begin{enumerate}
\item Create a first TCP socket labelled \textit{socket$_1$}.
\item Wait for \ins{a} connection\del{s} by making \textit{socket$_1$}  listen on \chg{some}{a} port.
...
\item Exchange data with the client through \textit{socket$_2$}. In the meanwhile, \textit{socket$_1$} can continue to wait for \ins{a new} connection\del{s}, and possibly create new sockets to exchange data with other clients. 
...
\alex{Can you make some room in the script. It is very compact. Add some comments also}
...
This backlog will not be actually used in \chg{this}{our} example.
...
\end{script}
\alex{Noury, I tried to the example, when I can run it only one. Any idea why?}
...
\alex{Maybe having as the caption a brief description of the method instead}
...
This mutex ensures that a single process can read or write the value of \ct!isRunning! (use of the \ct!critical:! message in \mthref{meth:isRunningRead} and \mthref{meth:isRunningWrite})\alex{what critical: does? I guess it prevent from two threads to be in the block provided to critical: right?}.
...
The creation of the serving process is the responsibility of the method \ct!start! (see the last line of \mthref{meth:start}).
\chg{The \ct!EchoServer>>>start! method}{This method} first checks \chg{if}{whether} the server is already running.
...
This value is a trade-off that depends on how fast \del{is} the server \ins{is} (\chg{varies according to}{depending on} the VM and the hardware) and the maximum rate of client connections requests.
The backlog size \chg{should}{has to} be \chg{big}{large} enough to avoid losing any connection request, but not too big to avoid wasting memory.
Finally \ct!EchoServer>>>start! method creates a process by sending the \ct!fork! message to the \ct![ self serve ]! block.
The created process has the same priority as the creator process (i.e. the one that performs the \ct!EchoServer>>>start! method\ins{, the UI thread if you have executed it from a workspace}\alex{to check, I am not sure}).
...
It \ins{is} worth noting that we ensure that the interaction socket is destroyed, whether we have exchanged data or not (timeout).
...
Once our socket stream \ins{is} connected to the server, we forge and send an HTTP GET query.
...
It will wait\del{s} \ins{for} a few seconds until data is received.
...
It \ins{is} worth noting that we chose these examples to make experiments straightforward and platform agnostic.
...
\chg{To experiment without dealing with HTTP, we can use Unix utilities.}{Network programming can easily scale up in complexity. Accessing a toolbox outside Pharo is often necessary to identify what is the source of an odd behavior. This sections list a number of survival Unit tools to deal with low level network operations.}
...
An alternative solution is to make the \ct!nc! server send text \ins{while?} you type.
...
\chg{In this chapter we presented how to}{This chapter introduces the} use TCP sockets and socket streams to develop both \ins{network} clients and servers. It has reviewed the survival kit of network programming:
...
\item The class \ct{Socket} is used to create low level sockets.
\item Numerical internet addresses are obtained from their name using the DNS, accessible from the class \ct{NetNameResolver}.
\item The messages \ct{sendData:} and \ct{receiveData} are the socket primitives to send and receive data.
\item A maximum waiting time is set with \ct{receiveDataTimeout:}.
\item \ct{SocketStream} is a buffered read-write stream that encapsulate a TCP socket.
