...
\begin{figure}[ht]\centering
	...
	\caption{Inter-Process Remote Communication Through Sockets \sd{would be good to have another representation for processes.}.}
...
At creation, the socket type (TCP or UDP) is provided to the socket plugin of the virtual machine \sd{do we need to know that?}.
To create a TCP socket, you need to evaluate the following message 
\begin{code}{}
Socket newTCP
\end{code}
Method \ct{newTCP} hands out the tcp type (which is stored in the class variable \ct!TCPSocketType! \sd{is it interesting to know that now?}) to set up a TCP socket.
...
By data we mean a \ct!ByteString!\footnote{One can send a \ct!ByteArray!, but the receiving socket will return a \ct!ByteString! however. \sd{unclear to me}}.
...
First, we retrieve the IP address of the \url{www.esug.org} server. \sd{does not look like this is the first thing we do.}
...
\sd{It would be good to have that as a setup so that the guy can directly try. Or use a unix command line that you mention afetr. The idea is to get really concrete use.}
We \sd{You should use - would be better} do use two different images, one that runs the server code and one for the client code. 
...
\subsection{Echo Server Class - \sd{we need a picture with processes}}\label{sec:echoServerClass}
...
The two last instance variables (\ct!isRunning! \sd{holding a boolean} and \ct!isRunningLock! \sd{holding a Mutex}) are used to manage the server process life-cycle while dealing with synchronization issues. 
...
Method \ct!EchoServer>>>serve! (see \mthref{meth:serve}) loop interacts with clients on connections \sd{unclear to me}.
...
\sd{would be good to have a pciture explaining socket stream and more importantly a API description of the most important methods}
...
To read data in chunks, \ct!SocketStream! does respond to the \ct!next:! message as illustrated by \scrref{script:socketStreamNext}.
