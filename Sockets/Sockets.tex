...
We first show through some examples how to use sockets for building both clients and servers. The notion of client and server are inherent to sockets: a server waits for a request emitted by a client.
...
Data reaches the destination in the same order of their sending.
...
Message \ct!connectTo:port:! attempts to connect the socket to the server using the address and port provided as parameters.
\nouryComment{SocketAddress can also store the port of the server. 
But, NetNameResolver does NOT support building socket address WITH port.} \ins{A port is a numerical value that comes with the address to designated a service. For example, 80 is the port expected by the HTTP server to receive a connection.}\alex{Noury, maybe you have a better explanation of what is a port}
...
Next, we send the \ct!receiveData! to our client socket to read the answer. Note that reading the answer is \emph{blocking}, meaning \ct{receiveData} returns when an response has been read. Then, the contents of variable \ct!data! is processed.
...
\item Wait for a connection by making \textit{socket$_1$}  listen on a port.
...
\item Exchange data with the client through \textit{socket$_2$}. In the meanwhile, \textit{socket$_1$} can continue to wait for a new connection, and possibly create new sockets to exchange data with other clients. 
...
This method first checks whether the server is already running.
...
It is worth noting that we chose these examples to make experiments straightforward and platform agnostic.
...
Network programming can easily scale up in complexity. Accessing a toolbox outside Pharo is often necessary to identify what is the source of an odd behavior. This sections list a number of survival Unit tools to deal with low level network operations.
