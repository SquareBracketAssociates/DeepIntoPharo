...
\paragraph{Callout.}
What we have just done is usually called an FFI \emph{callout}. \ct{NBFFICallout} is the class that defines behavior to perform such calls to external functions. We use the  message \ct{#cdecl:module:} to describe this call. 

This message has two arguments: the \emph{signature of the function} that is about to be called and the \emph{module or library} where to look for it which is the standard C library in this example. 
...
The third element is also an array which describes the arguments of the function when there's any. 

\paragraph{About conversion.} The return type is \ct{int} and not \ct{SmallInteger}. This is the same with function arguments. They are all describe in terms of C language. But you don't have to bother with this because the \Spock library does an automatic conversion between C types and Smalltalk types. So, when this message is executed, the return value will be a \ct{SmallInteger}. 
...
There are always three things to take into account when doing
callouts. First, of course, is the signature of the C function
we are going to call, the definition of the function from a C
point of view. Through this book we put in \emph{C header}
box. We also have to think on the way we access this function
from Smalltalk world, on the code that sends the message to
use the bounded C function. We put that in \emph{Library access}.
Finally, the complex part, is how we mix these two worlds,
the C and the smalltalk worlds. That's the \emph{Pragma declaration}.
This Pragma declaration is the code that describes
the C header and `binds' Smalltalk arguments to C arguments.
Let's look at a summary of our first example then:
...
\subsubsection{Passing an integer}

Let's try with a really simple function, {\sffamily abs}, which takes
...
First of all, in this case we are specifying \ct{abs}
instead of clock, of course. But besides that, the only
change is that we added an argument. When functions have
arguments you have to specify two things for each of them:
its type and the object that you want to have sent to the C
function. That is, in the arguments array, we put the type
of the argument, and after that, the name of the Smalltalk
variable we are going to send as argument. This type-and-name
pair will be repeated, separated by comma for each argument,
as we will show in the next examples. In this case, the
argument is a C integer, so you put {\sffamily int}. As the
object we want to pass is the same argument we received, we
directly put its name there in the declaration.
...
Before getting into the next example, a little thing
needs to be remarked. In the pragma declaration we
are expressing not one but \emph{two} different aspects:
the obvious one is the C function signature, the other
is which objects we are passing as arguments to the
C function when the method is invoked  to receive the
expected result. In this second aspect there are many
possibilities. In this case the argument of the C function
comes directly as an argument of the method: {\sffamily anInteger}.
But it is not always necessary to take the objects to
send as arguments. You can also put constants, {\sffamily
self} or any instance variable as an argument to the C
function. Suppose you want to add this {\sffamily abs}
wrapper to SmallInteger, so that you can do 

\dothis{-50 absoluteValue}

In that case you should add absoluteValue method to
SmallInteger, but instead of passing an argument you
directly pass {\sffamily self}!
...
Or in other circunstance you may want to have a wrapper that always
calls abs function with -45 value. Then you could directly
write
...
Notice that this time you don't put the type of the argument. It is
is also possible to pass an instance variable. We let that as a
really easy exercise.

\subsubsection{Passing strings}

Now we will call the very well known \ct{strlen} function.
This function requires a string as argument and returns its
...
In the signature department there are two small differences
between the C signature and the one we are providing
here in the Smalltalk array. The first difference is the const
keyword of the argument. For those not used to C, 
...
runtime. The second difference, an important one, is that as
argument we are providing \ct{String aString} instead
of \ct{char* aString}. It is important to remark again
that \Spock will take care of automagically doing the conversion
of the arguments which come from Smalltalk world, to C values.
But as this argument is not a common char pointer but a C string,
then a null terminator has to be present at its end. To make \Spock
know this is why you specify the argument as {\sffamily String}
and not as {\sffamily char*}.
\emph{This means that the string passed in the example is going
to be put in an external C \ct{char} array and that a null
termination character will be added to it (as Smalltalk strings
are not null terminated). Also, this array will
be automatically released after the call ends}. Conversely, Spock
will take the C result value of calling the C function and convert
it to a proper Smalltalk object, an Integer in this particular case.

%Javier, the example with strlen() function where you having char*
%argument type is not correct. One should use String type , instead of char* , because then NB will
%convert string into null-terminated C string before passing to
%function.
%Otherwise it only passing a pointer to first string character , but
%since in smalltalk string are not null terminated, the results can be
%different. It will work only for strings, which by accident having 0 as last character.
%So, you should use something like:
%
%CExamples stringLength: (someString copyWith: (Character value:0)
%or change the signature to use String type, i.e:
%
%int strlen(String arg)
%instead of
%int strlen(char* arg)
...
\subsection{Using C structures}

\subsection{Loading code from other libraries}
