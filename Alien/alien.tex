...
C function? and also, how do you write such binding? The libraries and macanisms in charge of doing this kind of work are usually called FFI, for Foreign Function Interface. In this chapter we will help you to use this interface by exposing some practical examples of
...
Another approach would be to invoke C libraries by defining a specific plugin, but its much more complex and it offers no important advantadge compared to the results of using bindings in the same language and with the same constructions you are used to.
...
has been running, but using the underlying OS function, that is, calling the 
\ct{clock} function. Its C declaration is:

\begin{code}{}
int clock (void)
\end{code}

To call \ct{clock} from the image, you will need to write a binding, which is a normal method with a \ct{<primitive:module:>}
...
You can define this method in any class, in instance or class side,
it doesn't make any difference. We  place it on the class side of a new one, called \ct{CExamples}. This way 
...
The important thing to notice in the binding is the 
\ct{#cdecl:module:} message and its arguments. This
message represents a call to an external function an has as
arguments the signature of the function that is about
to be called and the module where to look for it. 
The first argument is an array that describes the signature 
while the second is the name of the library, which in this case 
is the standard C library. The first element of the signature 
array is the C return type, and the second, the name of the called function.
The third and last element is also an array, which will describe the
arguments of the function when there's any. Notice that the
type of the return type is int, and not SmallInteger. This
will be the same with function arguments. They are all describe
in terms of C language.

But you don't have to bother about this. \Spock library does
an automatically conversion between C types and Smalltalk types.
So, actually, when this message is executed, the return value 
would be an SmallInteger. Just to let you know, this conversion
process for types from diferent languages is called marshalling.
We will see a lot more examples of automatic conversions in 
this chapter. 

Now, lets look at another example, and to make it more interesting
lets call the very well known \ct{strlen} function, which receives 
a string as argument and returns the amount of characters it has. 
Its C signature is:
...
example? First of all, in this case we are specifying strlen
