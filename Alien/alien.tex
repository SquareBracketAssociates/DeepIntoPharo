...
using the message \ct{nbCall:module:} message.\footnote{Note, that
...
In the above example, we define a class named \ct{CExamples} and
...
\ct{#nbCall:module:} is a method provided by NativeBoost package and
...
convert the return value from C to Smalltalk. \ct{#nbCall:module:}
...
The second argument, \ct{#module:}, is where you should specify a
module name or its handle, where to look up for given function.
NativeBoost provides a convenience method named \ct{CLibrary} to
...
\ct{NativeBoost forCurrentPlatform loadModule: 'nameOfModule'} loads
the module \ct{'nameOfModule'}.\footnote{Module names are platform
