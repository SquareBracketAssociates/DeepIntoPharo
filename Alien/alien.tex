...
To call \ct{clock} from the image, you should write a binding: a normal Smalltalk method annotated with the \ct{<primitive:module:>} pragma. This pragma specifies that a native call should be generated using the NativeBoostPlugin. The native class is described using the message \ct{cdecl:module:} message. Here is the full definition. We  explain the details below.
...
\item Third, we have to see how we organize the C calls from the Smalltalk side. Often we create a class that groups callout definitions. 
...
\paragraph{Example Analysis.}
Before getting into the next example, a little thing needs to be remarked. In the pragma declaration we
are expressing not one but \emph{two} different aspects: the obvious one is the C function signature, the other
is which objects we are passing as arguments to the C function when the method is invoked  to receive the
expected result. In this second aspect there are many possibilities. In this case the argument of the C function
comes directly as an argument of the method: \ct{anInteger}.

But it is not always necessary to take the objects to send as arguments. You can also put constants, \ct{self} or any instance variable as an argument to the C function. Suppose you want to add this \ct{abs} wrapper to SmallInteger, so that you can do \ct{-50 absoluteValue}

In that case you should add absoluteValue method to SmallInteger, but instead of passing an argument you
directly pass \ct{self}!
...
know this is why you specify the argument as \ct{ String}
and not as \ct{ char*}.
