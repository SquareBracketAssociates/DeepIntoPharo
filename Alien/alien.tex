...
NativeBoost methods \sd{which ones} to load external library (and hence obtain handle of it) before making any calls to it.
...
\sd{igor says:
Note that this time we donÕt put the type of the argument. This is
because for integer constants a NativeBoost automatically using 'int'
C type.
...
You can, of course take these contant values from header and put then
into your callout. But it is preferable to use a symbolic names for
constants,
which much less confusing than just bare numbers.
