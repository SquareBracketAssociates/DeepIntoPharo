...
\section{Power of 2}
Let's start with some simple maths. In digital world, information is encoded as powers of 2. Nothing really new. In Smalltalk raising a number to a power is performed by the method \ct{raisedTo:}. Here are some examples.
...
\sd{mmp: show an example, otherwise it is difficult to understand. Use for example 2r000001101 bitAt: 5 -> returns 0. Do the drawing you always do and it will be much clear. }\sd{ok later}\alex{I feel a drawing is not necessary, we give plenty of examples later on. We could maybe say that we will understand this formula with the two complement.}
...
The 10's complement of a positive decimal integer $n$ is 10 to the power of $k$ + 1, minus $n$, where $k$ is the number of digits in the \ins{textual} decimal representation of $n$. $Complement_{10}(n) = 10^{k+1} - n$.
...
\paragraph{Examples.}
\ins{Suppose we want to turn the subtraction $8 - 3 = 5$ into an addition}. The 10's complement of 3 is $9 - 3 + 1 = 7$. We add 7 to 8 and get 15. 
...
Now $98-60$. The 10's complement of 60 is $9-6$, $9-0$ \ie $39 + 1$ \ie 40. 
 ...
 We could have written $98-60 = 98 + 40 = 138 = 38$ \ugh{but this is an incorrect use of the $=$ mathematical operator}\ins{do you want to say that 138 is equal to 38 modulo 100?}.
...
Creating a two complement version of a number equals negating the number bits and adding one\ab{I have the impression we are repeating ourselves}.
