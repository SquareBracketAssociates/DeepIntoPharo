...
Smalltalk has a syntax for representing numbers in different bases. We write \ct{2r1101} where 2 indicates the base or radix \ja{the therm radix is not explained}, here 2, and the rest the number expressed in this base. Note that we could also write \ct{2r01101} or \ct{2r0001101} since this notation follows the convention that the least significant bit is the rightmost one. 

\ja{the following code need explanation}
\begin{code}{@TEST}
2r1101
	--> 13
...
\ja{the following paragraph is hard to read. aybe to much new terms. It also should have a reference to the  section '10's complement' (section 1.4 currently)}
...
\ja{what is the behavior if the bitShift is higher than the number of bits ? At the end of the chapter it is explained that a raisedTo: can change a SmallInteger into a LargeInteger. Is it also the case here ? We have to explicit it}
...
For example $Complement_{10}(8)= 10^1-8$,\luc{strange because according to the definition, shouldn't it be: $Complement_{10}(8)= 10^2-8$ which is wrong...} $Complement_{10}(1968)=10^{5}-1968=8032$
...
000000010 flipped is: 11111101 and we add one: so we get 11111110. The difference between  -2 (11111110) and 126 (01111110) is given by the most significant bit which conveys the integer sign. \ja{so what ?}
...
Remember a bitwise NOT is turning any 1s into 0s and conversely. You can use the bitString message to obtain the bit representation of a number. If you check the length of the printed results, you will notice that they are 31 bits long instead of 32. This is because in Pharo and most Smalltalk implementations, small integers are specially encoded and this encoding requires one bit. \ja{This is an interesting information. Why SmallInteger encoding requires one bit ? a figure explaining that could be cool}
...
\end{code}
 ...
 \ja{needs a small conclusion of this behavior}
 
\paragraph{Understanding some methods.} 
\ja{the following needs more explanation}
