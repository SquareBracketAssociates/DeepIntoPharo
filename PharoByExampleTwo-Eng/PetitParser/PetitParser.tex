...
Before we can use the browser we need to load PetitParser with the Glamour IDE.
Simply evaluate the following expression: \dc{I think this should be moved to the beginning of the chapter}
...
You can see our \clsind{ExpressionGrammar} and its subclasses as well as the \clsind{PPJsonGrammar} we defined earlier in this chapter.
...
\item[Graph]  shows the graphical representation of the rule.
  It is updated as the rule source is changed.\dc{show a screenshot}
\item[Example] shows an automatically generated example based on the definition of the rule.
  ...
  Note the \ct{Reload} button in the top-right corner that regenerates the example.\dc{show a screenshot}
\item[First] shows a list of parsers that can be activated directly
  after the rule started.\dc{I don't understand, maybe list some
    examples and say why}
\item[Follow] shows a list of parsers that can be activated directly
  after the rule finished.\dc{I don't understand either}
\end{description}
...
\item [Profile] shows how much time was spent in a particular parser during parsing of the input.
...
\item [Progress] shows  position in a string on the x-axis and time on the y-axis.
  ...
  If you see too many backwards jumps \eg as depicted in \figref{pp/progressBacktracking}, your parser performs a lot of backtracking which degrades the performance.
  In that case, you should reconsider the order of choice parsers or restructuralize your grammar. \dc{I don't understand this view. Maybe you could show 1 good and 1 bad diagram so we understand the difference?}
\end{description}
