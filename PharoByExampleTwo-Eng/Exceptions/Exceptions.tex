...
The \lct{<primitive: \emph{198}\,>} works the same way of the \lct{<primitive: \emph{199}\,>} we saw in the previous section. It always fails, however, its presence marks the executing context in a unique way. Moreover, the unwind block is stored the same way as the exception class and its associated handler. More explicitely, it is stored in the context of \mthind{BlockClosure}{ensure:}, that can be accessed from the block through  \lct{thisContext sender tempAt: 1 or 2}.

In the case where the block doesn't fail and doesn't have a non-local return, the \mthind{BlockClosure}{ensure:} message implementation is very easy to understand. It evaluates the block, store the result in the returnValue variable, evaluates the argument block and lastly return the result of the block previously stored. The complete variable is just here in order not to execute twice the argument block.

\paragraph{Ensuring a failing block.}

The \mthind{BlockClosure}{ensure:} message will trigger the argument block even if the block fails. In the following example, the \mthind{Bexp}{ensureWithOnDo} message will return 2 and execute 1. In the subsequent section we will carrefully look at where and what the block is actually returning and in which order are the block executed. 
...
There are three important things to note. First, the failing block and the main block are not fully executed because of the \mthind{Error}{signal} message. Secondly, the exception block is executed before the ensure block. Lastly, the start method will return the result of the exception handler block.

To understand how this works, we have to look at the end of the exception implementation. We finished the previous explanation on the \ct{handleSignal} method.
...
In our example, Pharo will execute the failing block, then will look for the next handler context, marked with \lct{<primitive: \emph{199}\,>}. As a regular exception, Pharo finds the exception handler context, and run the exceptionHandlerBlock. Now, the method finish by the \ct{return:} method. Let's have a look into it.
...
This is the method where the argument block of \mthind{BlockClosure}{ensure:} is executed. This method will look for all the unwind context between the context of the method \mthind{ContextPart}{resume:} and self, which is the sender of the \mthind{BlockClosure}{on:do:} context (in our case the context of \mthind{Bexp}{start}). When the method find an unwind context, the unwind block is executed. Lastly, it will trigger the \ct{terminateTo:} message.
...
Basically, this method will terminate all the context between thisContext and self, which is the sender of the \mthind{BlockClosure}{on:do:} context (in our case the context of \mthind{Bexp}{start}). Moreover, the sender of thisContext will become self, which is the sender of the \mthind{BlockClosure}{on:do:} context (in our case the context of \mthind{Bexp}{start}). It is implemented as a primitive, but the code below explains how it works.

Let's summarize what happened with this context stack figure, which represents the execution of the method  \mthind{Bexp}{ensureWithOnDo}
...
\begin{figure}[ht]\centering
        \includegraphics[width=9cm]{EnsureImpl}
...
There is also an implementation of \mthind{BlockClosure}{ensure:} for the non local return. Basically there is the same kind of lookup for unwind context as in the \mthind{ContextPart}{resume:} message that is triggered when we return the value in \mthind{ContextPart}{resume:through:}
