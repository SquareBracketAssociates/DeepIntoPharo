...
\emph{With the collaboration of \texbf{A. Plantec}}
...
\end{code}
So, one can use this expression  to configure the target of a corresponding setting. As an example the \cts{\#glyphContrast} preference could be declared as follow:
\begin{code}{}
...
The node identified by \cts{\#codeEditing} is created by sending the \mthind{SettingTreeBuilder}{group:} message to the builder with its identifier passed as argument.  Notice also that, as shown in \figref{fig:caseSensitiveFinds1}, the \cts{\#codeEditing} node is not at root because it is declared itself as a child of the \cts{\#codeBrowsing} node.
...
As in the following example of the \cts{\#appearance} group, you can indicate that no ordering should be performed by sending the \mthind{SettingNodeBuilder}{noOrdering} message to the parent node. Then its children are let in declaration order.
