...
Setting supports an object-oriented approach to \index{preference} preference definition and manipulation. What we want to express by this sentence is that:
\begin{enumerate}
\item each package or subsystem should define its own customization points (often represented as a variable or a class variable). The code of a subsystem then freely accesses such a customization value and uses it to change its behaviour to reflect the preference.
\item a subsystem describes its preferences so that the end-user can manipulate them. However, at not point in time, will the code of a subsystem  explicitly refer to setting objects to adapt its behaviour. 
\end{enumerate}
...
\item[Collecting setting for user presentation.] The Settings package defines tools to manage settings such as a \setbrowser that the user opens to change her/his preferences. The \setbrowser collects settings and uses their description to change the value of preferences.
...
\begin{figure}[tbh]
\begin{center}
\includegraphics[scale=0.28]{SettingBrowser}
...
Changing a \index{preference} preference value is simply done through the browser: each line holds a widget on the right with which you can update the value. The kind of widget depends on the actual type of the preference value. Whereas a preference value can be of any kind, the setting browser is currently able to present a specific input widget for the following types: \textit{Boolean}, \textit{Color}, \textit{FileName}, \textit{DirectoryName}, \textit{Font}, \textit{Number}, \textit{Point} and \textit{String}. A drop-list, a password field or a range input widget using a slider can also be used. Of course, the list of possible widgets is not closed as it is possible to make the setting browser support new kinds of preference values or use different input widgets. This point is explained in Section~\ref{sec:ExtendingTheSettingsBrowser}.

If the actual type of a setting is either \textit{String}, \textit{FileName}, \textit{DirectoryName},\textit{Number} or \textit{Point},  to change a value, the user has to enter some text in an editable drop-list widget. In such a case, the input must be confirmed by hitting the return key (or with cmd-s). If such a setting value is changed often, the drop-list widget comes in handy because you can retrieve and use previously entered values in one click! Moreover, in case of a \textit{FileName} or a \textit{DirectoryName}, a button is added to open a file name or a directory name chooser dialog.

Other possible actions are all accessible from the contextual menu.
Depending on the selected setting, they may be different. The two possible versions are shown in \figref{fig:TheSettingsBrowserPopupMenu}.
\begin{figure}[tbh]
\begin{center}
\begin{tabular}{cc}
\includegraphics[scale=0.5]{popup0.png} & \includegraphics[scale=0.5]{popup1.png} \\ 
\end{tabular} 
...
\end{figure}
\begin{itemize}
\item \textbf{Expand all (a)}: expand all the setting tree nodes recursively. It is also accessible via the keyboard shortcut \textit{cmd-a}.
\item \textbf{Collapse all (a)}: collapse all the setting tree nodes recursively. It is also accessible via the keyboard shortcut \textit{cmd-A}.
\item \textbf{Expand all from here}: Expand the currently selected setting tree node recursively.
...
\item \textbf{Display export action string}: a setting can be exported as a start-up action, this menu option allow to display how the start-up action is coded (Start-up action management is explained in Section \ref{sec:StartupActionManagement}).
\item \textbf{Set to default (d)}: set the selected setting value to the default one. It is useful if, as an example, you have played with a setting to observe its effect and finally decide to come back to its default.
...
You can filter the settings list by entering something in the search text field of the top bar of the SettingsBrowser. Then, only the settings whose name or description contains the text you have entered will be shown. The text can be a regular expression if the "Regexp" checkbox is checked.
...
All global preferences of Pharo can be viewed or changed using the \setbrowser. A preference is typically a class variable or an instance variable of a singleton. If one wants to be able to change a value from the \textit{SettingsBrowser}, then a setting must be declared for it.
...
A setting is declared by sending the message \ct{setting:} to the builder with an identifier passed as argument. Here is an example where the identifier is \cts{\#caseSensitiveFinds}: 
\begin{code}{}
CodeHolderSystemSettings class>>caseSensitiveFindsSettingsOn: aBuilder
	<systemsettings>
...
Sending the message \mthind{SettingTreeBuilder}{setting:} to a builder creates a \textit{setting node builder}\index{setting node builder} \index{setting node} which itself is a wrapper for a setting node. By default, the symbol passed as argument is considered as the selector used by the \setbrowser to get the preference value. The selector for changing the preference value is by default built by adding a colon to the getter selector (\ie it is \ct{caseSensitiveFinds:} here).
These selectors are sent to a target which is by default the class in which the method is implemented (\ie \ct{CodeHolderSystemSettings}). Thus, this one line setting declaration is sufficient if  \ct{caseSensitiveFinds} and \ct{caseSensitiveFinds:} accessors are implemented in \ct{CodeHolderSystemSettings}. 
...
For example,  the corresponding getter and setter accessors for the \ct{caseSensitiveFinds} setting are implemented in the class \cts{TextEditor}. Then, we should explicitly set that the target is \ct{TextEditor}. This is done by sending the message \mthind{PragmaSetting}{target:} to the setting node with the target class \ct{TextEditor} passed as argument as shown by the updated definition: 
\begin{code}{}
CodeHolderSystemSettings class>>caseSensitiveFindsSettingsOn: aBuilder
	<systemsettings>
	(aBuilder setting: #caseSensitiveFinds) 
...
This very short version is fully functional and enough to be compiled and taken into account by the \setbrowser as shown by \figref{fig:caseSensitiveFinds2}.
...
(aBuilder setting: #glyphContrast) 
    ...
    ...
\end{code}
This is simple, but unfortunately, declaring such a singleton target like this is not a good idea. This declaration is not compatible with the \textit{Setting style} functionalities (see Section~\ref{sec:SettingStylesManagement}). In such a case, one would have to separately indicate the target class and the message selector to send to the target class to get the singleton. Thus, as shown in the example below, you should use the \mthind{SettingNodeBuilder}{targetSelector:}\index{PragmaSetting!targetSelector:} message:
...
The way the \setbrowser builds a setting input widget depends on the actual value type of a preference. Having $nil$ as a value for a preference is a problem for the \setbrowser because it can't figure out which input widget to use.
...
The node identified by \cts{\#codeEditing} is created by sending the \mthind{SettingTreeBuilder}{group:} message to the builder with its identifier passed as argument.  Notice also that, as shown in \figref{fig:caseSensitiveFinds1}, the \cts{\#codeEditing} node is not at root because it has declared itself as a child of the \cts{\#codeBrowsing} node.
...
\item on the right, the \textit{Gradient} widget is checked, then the setting value is set to \ct{true} and as a consequence, the settings useful to set a gradient background are shown.
\end{itemize}
\begin{figure}[tbh]
\begin{center}
...
\end{code} 
The parent setting value is given by evaluating \ct{PolymorphSystemSettings class>>useDesktopGradientFill}. If it returns \ct{true}, then the children \cts{\#desktopGradientFillColor}, \cts{\#desktopGradientDirection}, and \cts{\#desktopGradientOrigin} are shown.
...
As an example, let's consider the full screen margin preference shown in the \figref{fig:range-setting}. Its value represents the margin size in pixels that is left around a window when it is expanded. 
...
Unfortunately, these values are not very handy. A programmer may expect another value. For example, some kind of \textit{strategy object} or a \ct{Symbol} which could directly serve as a selector. In fact, this second solution has been chosen by the \ct{RealEstateAgent} class maintainers.
If you inspect the value returned by \ct{RealEstateAgent usedStrategy} you will realize that the result is not a \ct{Symbol} among \cts{\#'Reverse Stagger'}, \cts{\#Cascade}, or \cts{\#Standard} but another symbol. Then, if you look at the way the window position strategy setting is really implemented you will see that the declaration differs from the basic solution given previously: the \textit{domainValues:} argument is not a simple array of {Symbol}s but an array of \ct{Associations} as you can see in the declaration below:
...
From the \setbrowser point of view, the content of the list is exactly the same and the user can not notice any difference because, if an array of Associations is given as argument to \mthind{SettingNodeBuilder}{domainValues:}, then the keys of the Associations are used for the user interface. 

Concerning the value of the preference itself, if you inspect \ct{RealEstateAgent usedStrategy}, you should notice that the result is a value among \cts{\#staggerFor:initialExtent:world:},  \cts{\#cascadeFor:initialExtent:world:} and \cts{\#standardFor:initialExtent:world:}. In fact, the values of the Associations are used to compute all possible real values for the setting.
...
Now, the question is if the desktop color setting is declared in \ct{PolymorphSystemSettings} and that the \ct{DefaultFonts} class allows fonts management? Where are all these settings declared and managed in general? 
...
\item the underlying model is not explicitly designed. The settings for one selection kind are grouped together in the \setbrowser, but corresponding preference values are declared as separated instance variables of \ct{ThemeSettings}. In the next section we see how to improve this first solution with a better design.
\end{enumerate}
\begin{figure}[tbh]
\begin{center}
...
\end{code}
\ct{TextSelectionPreference} is made of four instance variables. Two of them are the colors. If the \ct{mandatory} instance variable is set to \ct{false}, then the \ct{used} boolean instance variable can be changed. Instead, if the \ct{mandatory} is set to \ct{true}, then the used instance variable is set to \ct{true} and is not changeable. 

\ct{TextSelectionPreference} has also four class variables, one for each kind of possible text selection preference. The getters and setters have also to be implemented i to be able to manage these preferences from the \setbrowser.
...
The method \cts{settingInputWidgetForNode:} responsibility is to build the input widget for the \setbrowser. This method takes a \ct{SettingDeclaration} as argument. \ct{SettingDeclaration} is basically a model and its instances are managed by the \setbrowser. 
...
TextSelectionPreference class>>settingInputWidgetForNode: aSettingDeclaration 
	...
	^ (self theme newRowIn: self world for: uiElements) 
		 ...
		 yourself 
\end{code}
This method simply adds some basic elements in a row and returns the row. First, you can notice that the actual preference value, an instance of \ct{TextSelectionPreference}, is obtained from the \ct{SettingDeclaration} instance by sending \cts{\#preferenceValue} to it. Then, the user interface elements can be built based on the actual \ct{TextSelectionPreference} instance.
