...
%Blocks is a concept introduced by SCHEME in the seventies under the name lexical closures. 
...
If we execute the method \ct{blockOutsideTemp}, we now get a collection with 3, 3 and 3. This results means that each block in the collection now refers to a single variable \ct{temp}: we see that \ct{temp} is shared by the blocks.
...
The following examples show that escaping blocks jump to their home contexts but do not unwind the stack after this point \sd{explain that or change the sentence}. For example the previous example shows that the method \ct{start} was fully executed.
