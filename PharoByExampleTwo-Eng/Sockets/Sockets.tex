...
There different options to test the server of \Scrref{servingBasicExample}.
The first simple option is to use the \ct{nc} (netcat) utility discussed in \secref{netcat}.
First run the server script in a workspace.
Then, open a terminal and evaluate the following command line: 
\begin{code}
	echo "Hello Pharo" | nc localhost 9999
\end{code}

A pure Pharo alternative relies on using two different images: one that runs the server code and one for the client code. 
