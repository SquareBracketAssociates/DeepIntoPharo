...
The latter will take care of closing and destroying the underlying interaction socket.
...
Note that the client manages strings (ascii mode) or byte arrays (binary mode) have no impact on the server.
...
Involved entities in any conversation need a protocol that defines how to organize data into a sequence of bytes or characters.
...
Then, we read data in chunks of five characters that we display on the Transcript.
So, if the server sends a string with ten characters \ct!'HelloWorld'!, we will get on the Transcript \ct!Hello! on one line and \ct!World! on a second line.
...
Network programming can easily scale up in complexity. Using a toolbox outside Pharo is often necessary to identify what the source of an odd behavior is. This section lists a number of Unix utilities to deal with low level network operations.
...
The following snippet shows how to send \ct!'Hello from a client'! to a server on the local machine listening on port 9090.
\begin{code}{}
...
This command provides information on network interfaces and sockets of your computer.
It provides many statistics so one needs to use appropriate options to filter out information.
...
Why is \ct!lsof! useful, you would ask, if we already have \ct!netstat!?
The answer is that \ct!lsof! shows the link between processes and sockets.
So you can find sockets related to your program.
...
As mentioned in the introduction, we recommend using socket streams which are of a higher level and provide facility methods.
