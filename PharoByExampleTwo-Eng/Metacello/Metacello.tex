...
\begin{description}
\item{Baselines.} A baseline is a skeleton identifying abstractly the packages and subprojects that compose a configuration. Structural dependencies as well as code repositories are defined at the baseline level. The baseline can also specify the level of stability of the required components. 
\item{Versions.} A version is a set of package \emph{versions} that should be loaded. Often a version simply enriches a baseline with specific version information. 
\end{description}
...
ConfigurationOfCoolBrowser>>version01: spec 
       ...
       spec for: !\textbf{\#common}! do: [
              ...
              spec 
                     !\textbf{package:}! 'CoolBrowser-Core'  !\textbf{with:}! 'CoolBrowser-Core-MichaelJones.10';
...
Let us assume that version \ct{0.2} of our project consists of the package versions \mbox{\ct{'CoolBrowser-Core-MichaelJones.15'}} and \mbox{\ct{'CoolBrowser-Tests-JohnLewis.8'}} and a new package \ct{'CoolBrowser-Addons'} with version \ct{'CoolBrowser-Addons-JohnLewis.3'}. We specify this new configuration by creating the following method named \ct{version02:}.
...
A project is generally composed of several packages, which often have dependencies to other packages.  It is also likely that a certain package depends on a specific version of another package. Handling dependencies correctly is really important and  is one of the major benefits of Metacello. 
...
Now, let's continue with our example. First we modify it to use baselines: we create one method per baseline. 
\needspace{17ex}
\begin{code}{}
ConfigurationOfCoolBrowser>>baseline04: spec 				"optional"
       ...
       spec for: #common do: [
              spec !\textbf{blessing: \#baseline.}!    "mandatory to capture a baseline"
              spec repository: 'http://www.example.com/CoolBrowser'.
