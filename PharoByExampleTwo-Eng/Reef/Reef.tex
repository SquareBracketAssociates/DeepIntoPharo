...
Reef is a framework to build dynamic components. As a complement to Seaside, Reef does not replace it, instead, it makes easier the use of AJAX/Javascript inside it. A reef component can be embedded inside a Seaside one: we often call Reef components, component part for this reason. A Seaside component can be embedded inside a Reef one. 

In this Chapter, we will do a journey with Reef showing you its power. We are going to create a dynamic search of classes, to show basic AJAX interaction, then show some reef widgets. 
