...
A \ct{ROMondrianViewBuilder} models the Mondrian domain-specific language (DSL)\footnote{\url{http://www.moosetechnology.org/tools/mondrian}}. %It is mostly compatible with the original Mondrian language \ja{what is it?}.% (cf., Mondrian paper and website).
...
The produced visualization is given in \figref{abstractClasses}. It easily help identifying abstract classes that are not named as ``Array'' and the one that are abstract without having an abstract method.
