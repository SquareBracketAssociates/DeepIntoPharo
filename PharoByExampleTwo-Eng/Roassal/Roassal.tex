...
Visualization are usually composed of more than one data value. You can add more elements to a ROView by sending the message \ct{addAll:} with a collection of ROElements. You can later access them by using the \ct{elements} accessor. 
...
When a visualization contains more than one element it is hard to setup each position by hand. Roassal provides defined layouts to arrange ROElements. A layout in Roassal is a subclass of \ct{ROLayout}. Layouts are explained in  \secref{layouts}.
