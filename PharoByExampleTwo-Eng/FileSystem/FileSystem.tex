...
Finallty \ct{ensureDelete} deletes the file but does not raise error if the file does not exist. Similarly \ct{ensureDeleteAllChildren}, \ct{ensureDeleteAll} do not raise exception when the receiver does not exists. 
...
\ct{ensureCreateDirectory} verifies that the directory does not exist and only creates if necessary.
