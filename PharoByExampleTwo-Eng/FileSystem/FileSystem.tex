...
There are several operations on files such as \ct{delete} (deletes the file), \ct{deleteAll} (which deletes the directory and its contents), \ct{deleteAllChildren} (which only deletes children of a directory), \ct{deleteIfAbsent:} (which executes a block when the file does not exist, \ct{ensureDeleted} (which makes sure that the file will be deleted), and \ct{ensureDirectory} does not raise an error while \ct{createDirectory} does it. \ct{ensureFile} creates if necessary a file.

\sd{Stef here! And should comment the methods accordingly.}
