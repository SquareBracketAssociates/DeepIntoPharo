...
The class \ct{FileSystem} offers factory class-methods to offer access of different filesystems. 
...
A \ct{FileSystemDirectoryEntry} allows one to get the additional information of a file or a directory. These classes belongs to the 'FileSystem-Core' package, and are explained below in the chapter.
...
Please note that \ct{writeStream} overrides any existing file and \ct{readStream} throws an exception if the file does not exist. Forgetting to close stream is a common mistake, for which even advanced programmers regularly fall into. Closing a stream free low level resources, which is a good thing to do. The messages \ct{readStreamDo:} and \ct{writeStreamDo:} may be to use to not have to use \ct{close}. Consider:
...
Files may be copied and renamed using the messages \ct{copyTo:} and \mbox{\ct{renameTo:}.} Note that while \ct{copyTo:} expects another fileReference,  \ct{renameTo:} expects paths, pathnames or references.
...
When there is a need to access portion of a path, the message \ct{pathSegments} returns the full name cut into path elements, as strings. 
...
Finally \ct{ensureDelete} deletes the file but does not raise error if the file does not exist. Similarly \ct{ensureDeleteAllChildren}, \ct{ensureDeleteAll} do not raise exception when the receiver does not exists. 
