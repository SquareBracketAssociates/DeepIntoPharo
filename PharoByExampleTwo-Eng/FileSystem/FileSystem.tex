...
\ct{FileSystem} is the factory to access different filesystems. 
Sending the message \mthind{FileSystem}{disk} to \ct{FileSystem}, returns a file system as on your physical hard-drive. Sending \ct{memory} creates a file system at the system of the image. 
...
The message \mthind{FileReference}{workingDirectory} above returns a reference to the directory of your Pharo image. A references is an instance of the class \ct{FileReference}. References are the central objects of the framework and provide the primary mechanisms for working with files and directories. 
...
You can also copy and rename files using the methods \ct{copyTo:} and \ct{renameTo:}. Note that while \ct{copyTo:} expects another fileReference,  \ct{renameTo:} expects paths, pathnames or references.
...
To create a directory use the message \mthind{FileReference}{createDirectory}:
...
Then you can  copy the contents of the complete package-cache to that directory simply use \mthind{FileReference}{copyAllTo:}:
