...
Sending the message \method{disk} to \ct{FSFilesystem}, returns a file system as on your physical harddrive. Another less used possibility is \method{memory} to create a file system at the system of the image. 
...
The code above returns a reference to the working directory. References are instances of the class \ct{FSReference}. It  References are the central objects of the framework and provide the primary mechanisms for working with files and directories. 

In fact in Filesystem, four classes are central \ct{FSFilesystem}, \ct{FSReference}, \ct{FSLocator}, and \ct{FSDirectoryEntry}. You should do not use platform specific classes such as \ct{FSUnixStore} or \ct{FSWindowsStore}, there are internal classes. All code snippets below work on \ct{FSReference} instances.
