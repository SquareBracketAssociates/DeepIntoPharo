...
\ct{FSFilesystem} is the factory to access different filesystem. 

Sending the message \mthind{FSFilesystem}{disk} to \ct{FSFilesystem}, returns a file system as on your physical harddrive. Another less used possibility is \ct{memory} to create a file system at the system of the image. 
...
The message \mthind{FSReference}{workingDirectory} above returns a reference to the working directory. References are instances of the class \ct{FSReference}.  As we will see references are the central objects of the framework and provide the primary mechanisms for working with files and directories. 

Filesystem defines four classes that are important for the end-user: \ct{FSFilesystem}, \ct{FSReference}, \ct{FSLocator}, and \ct{FSDirectoryEntry}. You should do not use platform specific classes such as \ct{FSUnixStore} or \ct{FSWindowsStore}, there are internal classes. All code snippets below work on \ct{FSReference} instances.
...
Notice that \mthind{FSReference}{children} returns the files and folders. 
To recursively access all the children of the current directory you should use the message \mthind{FSReference}{allChildren}:
...
To find all jpeg files in \ct{working}, execute:
...
Navigating back to the parent is easy using the \mthind{FSReference}{parent} message:
...
\end{code} 
The methods \mthind{FSReference}{exists}, \mthind{FSReference}{isFile}, \mthind{FSReference}{isDirectory}, and  \mthind{FSReference}{basename} are defined on the \ct{FSReference} class. 

  
To get additional information about a filesystem entry, we should get an \ct{FSDirectoryEntry} using the message \mthind{FSDirectoryEntry}{entry}. Here are some examples:
...
\sd{this example seems conceptually bogus with the current state of FS}
...
\sd{in Pharo 1.4 we do not get 'Hello World' but a byte array}
...
\sd{renameAs: does not work since it calls unimplemented methods}
...
To then copy the contents of the complete package-cache to that directory simply use \mthind{FSReference}{copyAllTo:}:
...
To delete a single file, use the message \mthind{FSReference}{delete}:
\begin{code}{}
 (working / 'bar.txt') delete.
\end{code}

To delete a complete directory tree use \mthind{FSReference}{deleteAll}. Be careful with that one though.
...
\section{Design }
\sd{should check class comments and a uml diagram}
...
A filesystem is an interface to access hierarchies of directories and files. "The filesystem," provided by the host operating system, is represented by \ct{FSDiskFilesystem} and it's platform-specific subclasses. However, the user should not access them directly but using \ct{FSFilesystem} as we show previously. Other kinds of Filesystems are also possible. The memory filesystem provides a RAM disk filesystem where all files are stored as ByteArrays in the image. The zip filesystem represents the contents of a zip file.
...
Paths and filesystems are the lowest level of the Filesystem API. A \ct{FSReference} combines a path and a filesystem into a single object which provides a simpler protocol for working with files. It implements the same operations as \ct{FSFilesystem}, but without the need to track paths and filesystem separately:
