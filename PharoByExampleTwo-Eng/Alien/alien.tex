...
In the above example, we place it on the class side of \ct{CExamples}. 
...
What we just have done is usually called an FFI callout. 
\ct{NBFFICallout} represents a call to an external function.
...
It is important to know that  Spock will take care of automagically
...
which object is going to be passed as each argument. In this case, aString is
the first and anotherString is the second.

The next example adds a bit more complexity: we are going to pass a ByteArray
...
\dothis{ array := ByteArray new: 10.\
CExamples memorySet: array with: 100 for: 5.\
array}

\end{description}

As you can see, this time Spock took care of finding out the memory address where aByteArray lives and passing a pointer to it as the first argument. After executing the example you can see that the
byte array elements were actually changed. This is an important difference with strings: when you pass a string, a null character has to be added at the end, and for that reason it has to be copied to a bigger buffer. This means that strings are passed by-copy, while on the other hand byte arrays are passed by reference!
