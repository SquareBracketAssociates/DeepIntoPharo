...
\chapter{Inter-language communication}\label{cha:alien}
...
Suppose you want to use OpenGL in Pharo. Then
you would face a problem, because the OpenGL libraries
only know about C and nothing about Smalltalk. In
order to solve this kind of problems there is a
standard solution, which is to make a \emph{binding}
of each C library function to a Smalltalk method, and
then to call this methods from Smalltalk code.
But the remaining question is, how does a binding
call a C function? and also, how do you write such
binding? Doing this kind of work is usually called
...
In this chapter we are going to try to answer this kind
of questions, and give you some practical examples of
how to solve common problems faced when communicating
with other languages.

\section{Calling C code}

Some time ago, this wasn't an easy task, but the good
news is that since Pharo 1.4, this has become really
easy.

Suppose you want to know the amount of time the image
has been running, but in a C way, that is, calling the 
\texttt{clock} function. Its C declaration is:

\begin{code}{}
int clock (void)
\end{code}

To call clock from the image, you will need to write
a binding, which is a normal method with a {\sffamily <primitive>}
pragma, that describes the signature of the C function:
...
this method can be in any class, in instance or class side,
it doesn't make any difference. We will place it in a new
one, called CExamples, class side in order to make it
consistent through the chapter and easy to access.

Then, to know the time since the program started you can print

\dothis{CExamples ticksSinceStart}

but of course, this are the clock ticks, which aren't of much
use. The important thing to notice in the binding is the 
{\sffamily \#cdecl:module:} message and its arguments. This
message describes the signature of the function that is about
to be called. The first argument is an array that
describes the signature of the called function and its name.
The second one is the name of the library to look for the
called function, which this time is the standard C library.
The first element of the signature array is the C return type,
and the second, the name of the called function. The third
and last element is also an array, which will describe the
arguments of the function when there's any. Notice that the
type of the return type is int, and not SmallInteger. This
will be the same with function arguments.

If you look carefully
you will notice that the C integer returned was automatically
converted to a Smalltalk integer. We will see a lot more
examples of conversions in this chapter. 

Now, lets look another example, and to make it more interesting
lets call \texttt{strlen}, which receives a string as
argument and returns the amount of characters it has. Its
signature is:
...
Then the corresponding binding will be
...
Can you see the small differences with the previous
example? In this case, we are specifying strlen
instead of clock, of course. Besides that, the only
