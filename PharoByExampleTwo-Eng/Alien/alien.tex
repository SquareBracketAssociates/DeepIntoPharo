...
To call \ct{clock} from the image, you should write a binding: a normal Smalltalk method annotated with the \ct{<primitive:module:>} pragma. This pragma specifies that a native call should be performed using the NativeBoost plugin. \sig{You're not mistaken: a primitive is responsible only for running the previously generated native code, while code is generated at language side.} The external function call is described using the message \ct{nbCall:module:} message\footnote{Note, that this is not the only message which you can use for defining a callout. There are others, which we will discuss later, but for now use this form for introduction}. Here is the full definition. We  explain the details below.
...
The \ct{#nbCall:module:} is a method provided by NativeBoost package and defined for Object class (so you can use it anywhere), which is responsible for
...
Note, that the given message \ct{#nbCall:module:} should be invoked only by methods, which define the callout, because it requires a valid method context with a
