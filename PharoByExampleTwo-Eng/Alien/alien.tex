...
\item \sd{here: bed now} The second difference, an important one, is that as argument we are providing \ct{String aString} instead of \ct{char * aString}. 
With \ct{String aString}, \Spock will automatic do the conversion
of the arguments which come from Smalltalk world, to C values.

But as this argument is not a common char pointer but a C string, then a null terminator has to be present at its end. 

To make \Spock know this is why you specify the argument as \ct{String} and not as \ct{ char*}.
