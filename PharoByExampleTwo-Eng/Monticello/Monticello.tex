...
are the units of change. In this chapter we will use SmalltalkHub, but Squeaksource 3 can be use similarly. \emph{SmalltalkHub} is a central online repository in which you can store versions of your applications using \Mont. SmalltalkHub is the equivalent of \ind{SourceForge}, and \Mont the equivalent of CVS. 
...
Before we save our \ct{Perfect} package, we need to specify to \emph{where} we want to save it. A \emph{repository} is a package container, which may either be local to your machine or remote (accessed over the network). 
...
To add a new repository to the selected package, press the \button{+Repository} button. This will offer a number of choices of different kind of repository, including HTTP. For the rest of the chapter we will work with the \ct{package-cache} repository, as this is all we need to explore the features of \Mont.
...
A method or a class is in conflict if its definition has been altered. \figref{mergeTool} shows two conflicting methods in the class \ct{Integer}: \ct{isPerfect} and \ct{divisors}. A conflicting package element is indicated by being \underline{underlined}, \sout{barred}, or {\bf bold}. The full set of typeface conventions is as follows:
...
\item[Red=A method is conflicting.] A decision needs to be made to keep the proposed change or reject it. The proposed method \ct{Integer>>>>isPerfect} is in conflict with an existing definition in the image. The conflict can be resolved by right clicking on the method and \menu{Keep current version} or \menu{Use incoming version}.
\item[Right arrow=Repository replace current.] An element with a right arrow will be used and replace the current element in the image. In \figref{mergeTool} we see that \ct{Integer>>>isPerfect} from version 2 has been used.
...
\important{Do not name your top level package with a suffix (\eg \ct{Perfect}) that could match your subpackages. Do not define \ct{Perfect} as a required package of \ct{Perfect-Extensions} or \ct{PerfectTest}. You would run into in trouble as \Mont would save all the classes for three packages, though you only want two packages and an empty one at the top level.}
%-----------------------------------------------------------------

%\subsection{Making a Configuration} 

To build more flexible dependencies between packages, we recommend using a Metacello configuration (see \charef{metacello}). The \button{+Config} button creates a kind of configuration structure. The only thing to do is to add the dependencies.
...
\emph{Change set} is a \pharo built-in mechanism to define system patches. A change set is composed of global side effects on the system. A new change sets may be created and edited from the \emph{Change Sorter}. This tool is available from the \menu{World\go{}Tools} entry.
