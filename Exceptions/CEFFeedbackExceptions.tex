...
Modern programming languages, including \st, offer instead a dedicated exception-handling mechanism that greatly simplifies the way in which exceptional situations are signaled and handled.
...
In other words, once we have exceptions, any message send has the potential to not return to the sender: it may fail. 

%=================================================================
\section{Ensuring execution}

The \mthind{BlockClosure}{ensure:} message can be sent to a block to make sure that, even if the receiver block fails (\eg raises an exception) during its evaluation, the argument block will still be executed:
\begin{code}{}
...
The general mechanism is provided by the message \ct{on:do:}. It looks like this:
\begin{code}{}
!\emph{aBlock}! on: !\emph{exceptionClass}! do: !\emph{handlerAction}!
\end{code}
\noindent
\lct{\emph{aBlock}} is the code that could detect an abnormal situation and signal an exception; it is called the \emph{protected block}.   
...
\ct{\emph{exceptionClass}} defines the class of exceptions that \ct{\emph{handlerAction}} will be asked to handle.
...
The beauty of this mechanism lies in the fact that the protected block can be written in a straightforward way, \emph{without regard to any possible errors}. A different exception handler is responsible for taking care of anything that may go wrong.
...
If any other exception than \ct{FileStreamException} happens, the files would not be properly closed. 


%=================================================================
\section{Error codes --- don't  do this!}

Without exceptions supported by the environment, one (bad) way to handle a method that may fail to produce an expected result is to introduce explicit error codes as possible return values. In fact, in languages like C, code is littered with checks for such error codes, which often obscure the main application logic.
...
If \st would not provide exception-handling support, then the tiny example we saw in the previous section would be written something like this, using error codes:
...
\sd{CEF On the PDF version, I see this figure at the top of the page, cutting the previous code (bad)  example}
...
For example, 
because the exceptions \clsind{FileDoesNotExistException}, \clsind{FileExistsException} and \clsind{CannotDeleteFileException} are specific kinds of \clsind{FileStreamException}, they are represented as subclasses of \clsind{FileStreamException}, as shown in \figref{hierarchy}.
This notion of ``specialization'' allows us to associate an exception handler with a more or less general exceptional situation.
So, we can write:
\begin{code}{}
[ ... ] on: Error do: [ ... ]
...
\sd{CEF I thnk that with the above snippet of code, it may not be clear enough that they are three alternatives. It seems that are three consecutive sentences}
...
The message \ct{handles:} is also defines on a single exception and returns whether the receiver handles the exception.
...
Object>>>doesNotUnderstand: aMessage 
	 "Handle the fact that there was an attempt to send the given message to the receiver but the receiver does not understand this message (typically sent from the virtual machine when a message is sent to the receiver and no method is defined for that selector)."
...
\sd{CEF In the following line, as the plural 's' is outside the ct command, it was separated to the next line}
...
\sd{CEF I think that the different position of the square bracket is too subtle to notice the difference between both alternatives. We should use an example where this can be easier to see, or just omit the alternative}
...
\sd{CEF I could not find in my 1.4 the class Installer nor the method #installQuietly:. }
...
\sd{CEF this code gets cut between two pages in my PDF version}
\begin{code}{}
...
Note that to be sure to close the stream the \ct{stream close} should be guarded by an \ct{ensure:} invocation.
...
\sd{CEF I could not find this method in Pharo 14... it seems it was completely deprecated}
...
Deprecation>>>defaultAction
	...
	Preferences raiseDeprecatedWarnings ifTrue:
		[super defaultAction]
\end{code}
\sd{CEF The implementation in Pharo 14 is different, done indirectly through some class variables. I prefer this version for the example}
...
Up to now, we have presented the use of exceptions without really explaining deeply how they are implemented
...
So, the \st code that follows the primitive serves two purposes: it documents what the primitive does, and is available to be executed in the case that primitive would fail. 
...
To get the answer,  let's look at the definition of the class \ct{MethodContext}, whose instances make up the execution stack: 
...
This means that in addition to the named instance variables, objects of this class have some indexed slots.  In fact, every \ct{MethodContext} has an indexed slot for each argument of the method whose invocation it represents.  There are also additional indexed slots for the temporary variables of the method.

In the case that interest us, the arguments of the \ct{on:do:} message are stored in the indexed variables of the stack execution instance. To verify this, evaluate the following piece of code:
...
\sd{CEF The next paragraph is incomplete and I could not understand what did it intend to say}
In the protected block, we query the stack element that represents the protected block execution using . This execution was triggered by the \ct{on:do:} message execution. Hence, the stack element 

If you get some strange results using halt and inspect inside the protected block, pay attention that contexts are recycled by the Virtual Machine. Opening an explorer on thisContext will show you that the context sender is effectively the execution of the method \ct{on:do:}. 
...
\sd{CEF This seems to have very little difference with previous; I would use only one with the halt}
The last line explores a 2-element array that contains the exception class and the exception handler. 

Note that you can also execute the following code to obtain an explorer and that you can see that the exception class and the handler are stored in the first and second indexed instance variables of the method context object (a method context represents an element of the execution stack).
...
So, to summarize, the \ct{signal} method, with minimal assistance from the virtual machine, finds the context that corresponds to an \ct{on:do:} message with an appropriate exception class.   Because the execution stack is made up of Context objects that may be manipulated just like any other object, the stack can be shortened at any time.  This is a superb example of flexibility of \st.
...
In contrast, Notifications tell us that an event has occurred, but without the assumption that it is abnormal. \sd{CEF I am not sure of this; I think that any exception implies something abnormal. On errors, you cannot go on if unhandled, but Notifications are equally abnormal, as you say after}
...
\sd{CEF It may be worth considering also if you would like to store in the exception something more about its context, information additional to the one kept by default} 
...
Although this technique can be used in any programming language that support closures, since \st{} uses closures for \emph{all} its control structures, it is particularly natural to use this in \st{}.
...
\item An exception handler may terminate the protected block by sending \ct{return:} to the signaled exception; the resulting value of the protected block will be the argument supplied to \ct{return:}. 
...
\item Notifications are subclasses of Exception with the property that they can be safely resumed without the handler having to take any specific action.

\end{itemize}

\paragraph{Acknowledgments.}  We gratefully acknowledge Vassili Bykov for the raw material he provided. We also thank Paolo Bonzini for the \st implementations of \ct{ensure:} and \ct{ifCurtailed:}. We thank Hernan Wilkinson, Lukas Renggli, Christopher Oliver for their comments and suggestions.
\index{Bonzini, Paolo}
\index{GNU \st}
