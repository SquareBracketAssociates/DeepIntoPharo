...
Up to now, we have presented the use of exceptions without really explaining in depth how they are implemented at the Virtual Machine level. Note that since you do not need to know how exceptions are implemented to use them, you can simply skip this section on the first reading. Now if you are curious and really 
want to know how this is implemented at the Virtual Machine level, this section is for you. 
The mechanism is quite simple, making it worthwhile to know how it operates. Let's have a look at how exceptions are implemented at the Virtual Machine level and use stack execution elements (contexts) to store their information.
...
This code tells us two things: First, this method is implemented as a primitive, which means that  a primitive operation of the virtual machine is executed when this method is invoked.  VM primitives do not normally return: successful execution of a primitive terminates the method that contains the \lct{<primitive: \emph{n}\,>} instruction, answering the result of the primitive.
...
If you get some strange results using \ct{halt} and \ct{inspect} inside the protected block, note that contexts are recycled by the Virtual Machine. Opening an explorer on \ct{thisContext} will show you that the context sender is effectively the execution of the method \ct{on:do:}. 
...
We might think that the primitive 199 (the one used by \ct{on:do:}) is complex to write, but it is simple, because primitive 199 \emph{always} fails!
...
Now we know that when the method \ct{on:do:} is executed, the \ct{MethodContext} that makes up the stack frame is tagged and the handler and exception class 
are stored there. 
...
To summarize, the \ct{signal} method, with minimal assistance from the virtual machine, finds the context that correspond to an \ct{on:do:} message with an appropriate exception class.   Because the execution stack is made up of Context objects that may be manipulated just like any other object, the stack can be shortened at any time.  This is a superb example of flexibility of Pharo.
...
The \lct{<primitive: \emph{198}\,>} works the same way as the \lct{<primitive: \emph{199}\,>} we saw in the previous section. It always fails, however, its presence marks the executing context in a unique way. Moreover, the unwind block is stored the same way as the exception class and its associated handler. More explicitly, it is stored in the context (stack frame) of \mthind{BlockClosure}{ensure:}
method execution, that can be accessed from the block through  \lct{thisContext sender tempAt: 1}.

In the case where the block does not fail and does not have a non-local return, the \mthind{BlockClosure}{ensure:} message implementation is easy to understand. It evaluates the block, stores the result in the \ct{returnValue} variable, evaluates the argument block and lastly returns the result of the block previously stored. The \ct{complete} variable is just here to prevent  the argument block from being executed twice.
...
The \ct{return:} message will check if the context has a sender, and, if not, send a \ctCannotReturn} Exception. Then the sender of this context will call the \ct{resume:} message.
...
There is also an implementation of \mthind{BlockClosure}{ensure:} for the non local return. There is the same kind of lookup for unwound contexts as in the \mthind{ContextPart}{resume:} message that is triggered when we return the value in \mthind{ContextPart}{resume:through:}
