...
With the help of a series of examples, we shall explore all of these possibilities, and we shall also take a deep look into the internal mechanics of exceptions and exception handlers. The authors want to thank Camillo Bruni, Hernan Wilkinson and  Carlos Ferro for the feedback. 
...
If any other exception than \ct{FileStreamException} happens the files are not properly closed. 
...
So, we can write different expressions depending on the level of granularity we want:
...
The usefulness of resuming an exception is illustrated by the following functionality which loads. When installing packages, warnings may be signaled. Warnings should not be considered fatal errors, so we should simply ignore the warning and continue installing. 
The class \ct{PackageInstaller} does not exist but here is a sketch of a possible implementation.
...
\ct{ensure:} argument is always executed while \ct{ifCurtailed:} argument is only executed when its receiver execution led to a  stack got unwind. 
...
\ct{ifCurtailed:} is a watchdog that reacts to stack abnormal behavior.
...
The following expression shows that as previously when an error occurs the handler associated with the error is executed before the ensure: argument. Here the expression prints 	\ct{should show error first}, then \ct{then should show ensure} and it returns 4. 
...
The first preference simply causes a warning message to be written on the \ct{Transcript}.  The second preference asks for an exception to be signaled, which is accomplished by \super-sending \ct{defaultAction}. Note that this implementation was changed in the current distribution of Pharo to use Setting a modular preference mechanism (See Chapter~\ref{cha:setting}).
...
In the protected block, we query the stack element that represents the protected block execution using \ct{thisContext sender}. This execution was triggered by the \ct{on:do:} message execution. The last line explores a 2-element array that contains the exception class and the exception handler. 


If you get some strange results using halt and inspect inside the protected block, pay attention that contexts are recycled by the Virtual Machine. Opening an explorer on \ct{thisContext} will show you that the context sender is effectively the execution of the method \ct{on:do:}. 
...
So we saw that \ct{on:do:} execution stores the exception class and its handler on the method context (execution stack frame). Note that this is not specific to \ct{on:do:} but 
any message execution stores argument on the stack frame. 
...
But it  is trivial, because primitive 199 \emph{always} fails!
Because the primitive always fails, the \st{} body of \ct{on:do:} is always executed.
However, the presence of the \ct{<primitive: 199>} annotation marks the executing context in a unique way. 
...
Now, if an exception is signaled further up the stack, the method \ct{signal} can search the stack to find the appropriate handler. This is what the following code is doing:
...
The \lct{<primitive: \emph{198}\,>} works the same way of the \lct{<primitive: \emph{199}\,>} we saw in the previous section. It always fails, however, its presence marks the executing context in a unique way. Moreover, the unwind block is stored the same way as the exception class and its associated handler. More explicitely, it is stored in the context (stack frame) of \mthind{BlockClosure}{ensure:}
method execution, that can be accessed from the block through  \lct{thisContext sender tempAt: 1}.

In the case where the block doesn't fail and doesn't have a non-local return, the \mthind{BlockClosure}{ensure:} message implementation is very easy to understand. It evaluates the block, stores the result in the \ct{returnValue} variable, evaluates the argument block and lastly returns the result of the block previously stored. The \ct{complete} variable is just here to prevent to execute twice the argument block.
...
\sd{clŽment pourquoi tu as besoin de dŽfinir des mŽthodes?}
\begin{code}{}
Bexp>>mainBlock 
...
The \ct{return:} message will check if the context has a sender, and, if not, send a CannotReturn Exception. Then the sender of this context will call the \ct{resume:} message.
...
Basically, this method terminates all the contexts between thisContext and self, which is the sender of the \mthind{BlockClosure}{on:do:} context (in our case the context of \mthind{Bexp}{start}). Moreover, the sender of thisContext will become self, which is the sender of the \mthind{BlockClosure}{on:do:} context (in our case the context of \mthind{Bexp}{start}). It is implemented as a primitive, but the code below explains how it works.
...
There is also an implementation of \mthind{BlockClosure}{ensure:} for the non local return. Basically there is the same kind of lookup for unwound contexts as in the \mthind{ContextPart}{resume:} message that is triggered when we return the value in \mthind{ContextPart}{resume:through:}

\begin{code}{}
ContextPart>>resume: value through: firstUnwindCtxt
	"Unwind thisContext to self and resume with value as result of last send.
	 Execute any unwind blocks while unwinding. ASSUMES self is a sender of thisContext."
