...
The mechanism is quite simple, making it worth to know how it operates. Let's have a look at how exceptions are implemented at the Virtual Machine level and use stack execution elements (contexts) to store their information.
...
We see that \ct{on:do:} execution stores the exception class and its handler on the method context (execution stack frame). Note that this is not specific to \ct{on:do:} but 
...
We might think that the primitive 199 (the one used by \ct{on:do:}) is complex to write. 
...
However, the presence of the \ct{<primitive: 199>} annotation marks the execution context in a unique way. 
