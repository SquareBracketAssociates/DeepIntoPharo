...
Now if we do not specify any \ct{default}, it will take by default everything, hence it will be equivalent in our case with \ct{spec group: 'default' with: #('Core' 'Tests')}

We believe that by default it is good to load the tests as well. This is why, either we explicitly put the \ct{Tests} group in the \ct{default} group, or we do not specify a \ct{default} at all.
...
We've named the project reference \ct{CoolBrowser ALL}. The name of the project reference is arbitrary, you can select the name you want, although is it recommended to put a name that makes sense to that project reference.  In the specification for the \ct{CoolToolSet-Core} package, we've specified that \ct{CoolBrowser ALL} is required. As will be explained later, the message \ct{project:with:} allows one to specify the exact version of the project you want to load.
...
In this baseline, \ct{package: 'Phexample' with: [spec repository: 'http://www.squeaksource.com/phexample' ];}
...
\begin{description}
\item[stable.] A symbolic version that specifies the stable literal version for a particular platform and version of such a platform. The stable version is the version that should be used for loading. With the exception of the \ct{bleedingEdge} version (which has a pre-defined default defined%\sd{what does it means: that default load the bleedingEdge?})
, you will need to edit your configuration to add the \ct{stable} or \ct{development} version information. \emph{I want a certified version for the platform.}
