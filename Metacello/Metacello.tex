...
\paragraph{Selective Loading.} By default, the \mthind{load}{load} message loads all the packages associated with the version (as we will see later, we can change that by defining a particular group called 'default'). If you want to load a subset of the packages in a project, you should list 
...
We often get at least two groups in a configuration: \ct{'Core'} (the real code) and the \ct{'Tests'} (the associated tests).  The question is what should be loaded by the \ct{'default'} group (remember, this is what gets loaded when you do not specify anything in the dependency).

By saying \ct{spec group: 'default' with: #('Core')} we say that by default we do not load tests. 

Now if we do not specify any 'default', it will take by default everything, hence it will be equivalent in our case with \ct{	spec group: 'default' with: #('Core' 'Tests')}
