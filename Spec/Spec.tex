...
Write getter methods since they will be needed by the presenter. It's still miss the entry point to specify the list of methods to browse. Let us name such method \ct{methods:} and define it as follow.  
...
\sd{behavior: for textModel looks strange}
...
Now if you evaluate the following snippet, you should have a window as shown in \figref{fig:methodBrowser1}.
...
\end{code}
The list should automatically be updated, and looks as in \figref{fig:methodBrowser2}.
\begin{figure}[ht]
\begin{center}
...
%\newpage
And if you select a method its source code should appear, as in \figref{fig:methodBrowser3}
...
So we will define the class method \ct{title} as follow.
%
...
So let's add the following line into \ct{MethodBrowser>>initialize}
\begin{code}{}
