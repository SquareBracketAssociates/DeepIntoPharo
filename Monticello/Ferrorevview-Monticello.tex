...
are the units of change. \emph{\sqsrc{}} is a central online repository in which you can store versions of your applications using \Mont. \sqsrc is the equivalent of \ind{SourceForge}, and \Mont the equivalent of CVS. 

In this chapter, you will learn how to use \Mont and \sqsrc to manage your software. We have already met \Mont briefly in earlier chapters\footnote{``A first application'' and ``The Pharo programming environment''}.
...
In the \MCB,  a dirty package can be recognized by an asterisk (\ct{*}) preceding its name.  This indicates which packages have uncommitted changes, and therefore need to be saved into a repository if those changes are not to be lost. Saving a dirty package cleans it. \ab{Why is it that when I click \button{Changes} on a dirty package, I sometimes get a dialog back that says ``no changes''?  I found this very confusing.  How can I see \emph{why} \Mont thinks a package is dirty?  How can I see \emph{which} repository it was originally loaded from or saved to?}
...
\sd{CEF I have a differet bar of buttons, History is not present, although it is on the right button menu}
...
\end{code}
\sd{CEF I have an option for "Use Polymorph difference tools", i guess it is that}
...
\sd{CEF I think it is not "dependent" packages which are saved (they would be packages that depend on Pier-All), but dirty packages that Pier-All depends}
...
\sd{CEF Again, I think that are "required" packages, not dependent. Besides, I don't see them bold in my browser, but I see an arror to expand -All and see the other as children}
...
\end{enumerate}
\sd{CEF I don't have any change in the DiffPerfect set. Instead, the system created a new change set as Perfect-CarlosEFerro22}
...
\indmain{\sqsrc} is an online repository that you can use to store your \Mont packages. An instance is running and accessible from \url{http://www.squeaksource.com}. At the time this chapter is being written, over 1500 projects are registered on \sqsrc and nearly 2000 people have an account. \figref{squeaksource} shows the main web page.  
\sd{CEF Now, there are mor than 3100 project and more than 3600 members. And it has a red warning about disabling the creation of new projects, and the need to move to a Gemstone repository for that}
...
These files are commonly called ``mcz files'' as they carry the extension .mcz.
...
\begin{itemize}
\item \Mont packages are mapped to Smalltalk categories and method protocols.
...
\index{package-cache}
\item Saved packages are cached locally in a directory called \ct{package-cache}.
