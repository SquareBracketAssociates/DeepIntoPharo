...
Now to see that a return is really escaping the current execution, let us build a slightly more complex call flow. We define four methods among which one (\ct{defineBlock}) creates an escaping block,  one (\ct{arg:}) evaluates this block and one (\ct{evaluatingBlock:}) that executes the block. Pay attention that to stress the escaping behavior of a return we defined \ct{evaluatingBlock:} so that it endlessly loops after evaluating its argument.
...
\important{Non local return \ct{[^ ...]} returns to the sender of the block home context. It means to the method execution point that called the one that created the block.}
...
Now let's look at the case of block nesting and its impact on home contexts.
...
\item \ct{thisContext closure == b1} shows that the context inside the execution of the block \ct{b1} has a pointer to \ct{b1}. The outer context of b1 is \ct{homeContext}. Nothing new because \ct{b1} is defined during the execution starting after the assignment. The home context of \ct{b1} is the same than its outer context.

\item Inside \ct{b2} execution, the current context points to \ct{b2} itself since it is a closure. More interesting 
the outer context of the closure in which b2 is defined \ie b1 points to \ct{homeContext}. Finally the home context of \ct{b2} is \ct{homeContext}. This last point shows that the all the nested blocks have a separate outer context but they share the same home context.
\end{itemize}
