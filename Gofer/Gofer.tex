...
\begin{script}[]{Getting the package list.}
(Gofer new
	squeaksource3: 'Kozen';
	allResolved)
...
\begin{script}[]{Fetching all the refactoring packages from Pharo2.0 repository}
| go |
go := Gofer new.
...
(go allResolved select: [:each | 'Refactoring*' match: each packageName])
	do: [:pack| 
		...
		package: pack packageName; fetch]
\end{script}
...
\begin{script}[]{How to publish Fame to SmalltalkHub}
|go repo|
